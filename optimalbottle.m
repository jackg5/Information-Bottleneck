...
% * Hga = the horizontal axis position, H(T) - alpha H(T|X)
function [Qtgx, Qt, L, Ixt, Iyt, Hga] = optimalbottle(Pxy, ...
    ...
    % Set defaults for 6th argument
    if nargin < 6
        debug = false;
    end
    % Set defaults for the 7th argument
    if nargin < 7
        merge = true;
    end
    ...
    while ~optimized
       ...
       [newQtgx, newQt, newL, newIxt, newIyt, newHgt, newHtgx] = ...
           bottleneck(Pxy, beta, alpha, epsilon, true);
    ...
    % If we are using the DIB, we need to merge clusters to see if
        ...
        while keepMerging
            ...
            for i = 1:length(T1)
                ...
                [mergedQtgx, mergedQt, mergedL, ...
                    mergedIxt, mergedIyt, ...
                    mergedHgt] = bottleneck(Pxy, beta, alpha, ...
