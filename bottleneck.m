...
%
% From this, we have the results obtained by
...
function [Qtgx, Qt, L, Ixt, Iyt, Ht, Htgx] = bottleneck(Pxy, ...
    ...
    % true when we are in the DIB, IB
    forceSingleton = alpha == 0; % DIB 
    forceSingleton = forceSingleton || (alpha == 1); % IB
    ...
    while ~converged
        ...
        [newQtgx,Z] = updateQtgx(Qygt, Qt, Pygx, beta, alpha);
...
function [Qtgx, Z] = updateQtgx(Qygt, Qt, Pygx, beta, alpha)
